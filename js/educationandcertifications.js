...
function setEducationsAndCertificates() {
    ...
    setTimeout(() => {
        const qualificationsUrl = API_URLS.QUALIFICATION;
        fetch(qualificationsUrl, { signal: signal }).then(response => {
            if (!response.ok) {
                throw new Error('');
            }
            ...
            trackError(headerTitle, `Fetch ${getEndpoint(qualificationsUrl)}`, error.message);
        });
    }, DELAY);
