...
function setWorkExperiencesAndAwards() {
    ...
    awardIcons = {};
    setTimeout(() => {
        const workExperiencesAndAwardsUrl = API_URLS.WORK_EXPERIENCE_AND_AWARDS;
        fetch(workExperiencesAndAwardsUrl, { signal: signal }).then(response => {
            if (!response.ok) {
                throw new Error('');
            }
            ...
            trackError(headerTitle, `Fetch ${getEndpoint(workExperiencesAndAwardsUrl)}`, error.message);
        });
    }, DELAY);
}
...
function fetchMedia(imageItem) {
    ...
    fetch(logoUrl, { signal: signal }).then(response => {
        ...
        trackError(headerTitle, `Load ${imgName}`, error.message);
    })
}
...
function fetchAwardsLogo(logoData) {
    ...
    fetch(logoUrl, { signal: signal }).then(response => {
        ...
        trackError(headerTitle, `Load ${logoData.icon}`, error.message);
    })
}
