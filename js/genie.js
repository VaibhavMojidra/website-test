let fadeInAnimationTimeOut
let fadeOutAnimationTimeOut
let dismissTimeOut
const TOAST = {
    SHORT: 2500,
    LONG: 4200
...
let editors = [];
const EDITOR_LIGHT_THEME = "crimson_editor";
const EDITOR_DARK_THEME = "dracula";
let editorTheme = EDITOR_LIGHT_THEME;
...
document.addEventListener('DOMContentLoaded', function () {
    setTheme(function () { editorTheme = EDITOR_LIGHT_THEME }, function () { editorTheme = EDITOR_DARK_THEME });
...
function addAboutQuestion() {
    questions.push({ questions: ["Hi", "Hello", "Hey", "Hello Genie", "Hey Genie", "Hi Genie", "Can you introduce yourself?", "How would you describe yourself?", "Tell me something about yourself.", "Can you give me a brief overview of who you are?", "Tell me about yourself"], type: QUESTION_TYPES.ABOUT })
}
...
function initializeEditors() {
    ...
    document.querySelectorAll(".code-block").forEach((div, index) => {
        ...
        setTimeout(() => {
            let totalLines = editor.session.getLength();
            let lineHeight = editor.renderer.lineHeight || 18;
    ...
    articleDataList.forEach(function (item) {
        switch (item.type) {
            ...
            case ARTICLE_DATA_TYPES.CODE: {
                const codeBlock = generateCodeBlock(item.code, item.lang);
                htmlContent.push(codeBlock);
            }
                break;
            case ARTICLE_DATA_TYPES.FOLDER_STRUCTURE: {
                const folderBlock = generateFolderStructure(item.folderTree);
                htmlContent.push(folderBlock);
            }
                break;
            case ARTICLE_DATA_TYPES.TABLE: {
                const tableBlock = generateTable(item.html);
                htmlContent.push(tableBlock);
            }
                break;
            case ARTICLE_DATA_TYPES.SPACER: {
                const spacerBlock = generateSpacer(item.size);
                htmlContent.push(spacerBlock);
            }
                break;
            case ARTICLE_DATA_TYPES.ATTACHMENT: {
                const attachmentBlock = generateAttachments(item.attachments);
                htmlContent.push(attachmentBlock);
            }
                break;
            case ARTICLE_DATA_TYPES.LINKS: {
                const linksBlock = generateLinks(item.links);
                htmlContent.push(linksBlock);
            }
                break;
            case ARTICLE_DATA_TYPES.BUTTONS: {
                const buttonsBlock = generateButtons(item.buttons);
                htmlContent.push(buttonsBlock);
            }
                break;
            default: { }
        }
...
function generateButtons(buttons) {
    return `<div class="buttons-container">${buttons.map(item => `<div class="action-button" onclick="openInNewTab('${item.link}')">${item.text}</div>`).join('')}</div>`;
}
...
function generateLinks(links) {
    ...
    links.forEach(function (item) {
        const { displayText, link } = item;
        const iconSuffix = giveIconClassSuffix(link);
        htmlParts.push(`<div class="link-item" onclick="openInNewTab('${link}')"><div class="link-item-icon link-icon${iconSuffix}"></div><div class="link-item-text">${isNullOrEmpty(displayText) ? link : displayText}</div></div>`);
    })
    return `<div class="links-container">${htmlParts.join("")}</div>`;
}

function giveIconClassSuffix(link) {
    const domainToClass = {
        "youtube.com": "-youtube",
        "github.com": "-github",
...
function generateAttachments(attachments) {
    const attachmentGenerators = {
        img: generateImageAttachment,
        video: generateVideoAttachment,
        doc: generateDocAttachment
    };

    return `<div class="attachement-container">
            ${attachments.map(item => {
        const generator = attachmentGenerators[item.type.toLowerCase()];
        return generator ? generator(item) : '';
    }).join('')}</div>`;
}

function generateImageAttachment(attachment) {
    const backgroundImage = getRandomItem(["attachment1.jpg", "attachment2.jpg", "attachment3.jpg", "attachment4.jpg", "attachment5.jpg"]);
    const backgroundUrl = `${BASE_URL}/assets/img/${backgroundImage}`;
    return createAttachmentUi(attachment.link, backgroundUrl, "view-action", true);
}

function generateVideoAttachment(attachment) {
    const backgroundImage = getRandomItem(["attachment1.jpg", "attachment2.jpg", "attachment3.jpg", "attachment4.jpg", "attachment5.jpg"]);
    const backgroundUrl = `${BASE_URL}/assets/img/${backgroundImage}`;
    const { link, isDownloadable } = attachment;
    return createAttachmentUi(link, backgroundUrl, "play-action", isTrue(isDownloadable));
}

function generateDocAttachment(attachment) {
    const backgroundUrl = `${BASE_URL}/assets/img/doc.jpg`;
    return createAttachmentUi(attachment.link, backgroundUrl, "doc-action", true);
}

function createAttachmentUi(link, backgroundUrl, actionClassName, isDownloadable = true) {
    return `<div class="attachement-item"><img src="${backgroundUrl}" class="attachement-bg"><div class="attachement-overlay"></div><div class="attachement-actions-container"><div class="attachement-action" onclick="openInNewTab('${link}')"><div class="${actionClassName}"></div></div>${isDownloadable ? `<div class="attachement-action" onclick="downloadFile('${link}')"><div class="download-action"></div></div>` : ''}</div></div>`;
}
...
function generateTable(tableHtml) {
    return `<div class="table-block">${tableHtml}</div>`;
}

function generateFolderStructure(folderTree) {
    const folderBlockContent = folderTree.type == "file" ? generateFileBlock(folderTree) : generateFolderBlock(folderTree);
    return `<div class="folder-block">${folderBlockContent}</div>`;
}

function generateFolderBlock(folderItem) {
    const { name, comment, content } = folderItem;
    const folderBlockHtml = [];
    const contentList = toFolderContent(content);
    let commentBlock = "";
    if (toString(comment) != "") {
        commentBlock = `<div class="folder-comment"><i># ${comment}</i></div>`;
    }
    folderBlockHtml.push(`<div class="folder-item"><div class="folder-file-name-container"><div class="folder"></div><div class="folder-name">${name}</div>${commentBlock}</div>`)
    folderBlockHtml.push(contentList.length != 0 ? '<div class="folder-content">' : "");
    contentList.forEach(item => {
        if (item.type == "file") {
            folderBlockHtml.push(generateFileBlock(item));
        } else {
            folderBlockHtml.push(generateFolderBlock(item));
        }
    });
    folderBlockHtml.push(contentList.length != 0 ? "</div>" : "");
    folderBlockHtml.push(`</div>`);
    return folderBlockHtml.join('');
}

function toFolderContent(content) {
    return content == null ? [] : content;
}

function generateFileBlock(fileItem) {
    const { name, comment } = fileItem;
    let commentBlock = "";
    if (toString(comment) != "") {
        commentBlock = `<div class="file-comment"><i># ${comment}</i></div>`;
    }
    return `<div class="folder-file-name-container file-item"><div class="file"></div><div class="file-name">${name}</div>${commentBlock}</div>`;
}
...
function onToggleToLightTheme() {
    editorTheme = EDITOR_LIGHT_THEME;
    editors.forEach(editor => {
        try {
            editor.setTheme(`ace/theme/${editorTheme}`);
        } catch (error) { }
...
function downloadFile(url, filename = 'download') {
    if (isExistDownloadFile(url)) {
        showToast("Already in download queue", TOAST.SHORT);
        return;
    }
    addDownloadFile(url);
    showToast("Started download..", TOAST.SHORT);
    fetch(url)
...
function addDownloadFile(file) {
    if (!isExistDownloadFile(file)) {
        downloadingFiles.push(file.toLowerCase().trim());
    }
}

function removeAllDownloadFiles(file) {
    downloadingFiles = downloadingFiles.filter(f => f.toLowerCase().trim() !== file.toLowerCase().trim());
}

function isExistDownloadFile(file) {
    return downloadingFiles.some(f => f.toLowerCase().trim() === file.toLowerCase().trim());
}

function showToast(toastMessage = "Hi", toastDuration = TOAST.SHORT) {
    let toastContainer = getId('toast-container')
    toastContainer.innerHTML = toastMessage

    if (fadeInAnimationTimeOut) {
        clearTimeout(fadeInAnimationTimeOut);
    }
    if (fadeOutAnimationTimeOut) {
        clearTimeout(fadeOutAnimationTimeOut);
    }
    if (dismissTimeOut) {
    ...
    fadeInAnimationTimeOut = setTimeout(() => {
        toastContainer.style.opacity = "1"
    }, 10)

    fadeOutAnimationTimeOut = setTimeout(() => {
