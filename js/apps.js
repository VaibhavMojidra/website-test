...
function setApps() {
    setTimeout(() => {
        const appsUrl = API_URLS.APPS;
        fetch(appsUrl, { signal: signal }).then(response => {
            if (!response.ok) {
                throw new Error('');
            }
            ...
            trackError(headerTitle, `Fetch ${getEndpoint(appsUrl)}`, error.message);
        });
    }, DELAY);
}
...
function fetchMedia(imageItem) {
    ...
    fetch(url, { signal: signal }).then(response => {
        ...
        trackError(headerTitle, `Load ${getEndpoint(url)}`, error.message);
    })
}
